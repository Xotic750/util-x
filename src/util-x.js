...
(function (globalThis, window, module, define, base, Undefined) {
    ...
    var testShims = true,
        ...
        */
        ...
        isErrorTypePrototype = function (inputArg) {
            ...
            case protoTypeError:
                /* falls through */
            case protoSyntaxError:
                /* falls through */
            case protoRangeError:
                /* falls through */
            case protoEvalError:
                /* falls through */
            case protoReferenceError:
                /* falls through */
            case protoUTIError:
                break;
            default:
        ...
        toClass = (function () {
            try {
                if (testShims ||
                        pOToString.call() !== classUndefined ||
                        pOToString.call(null) !== classNull ||
                ...
                return function (inputArg) {
                    ...
                    if (type === 'undefined') {
                        ...
                        val = classArguments;
                    } else if (inputArg === protoNumber) {
                        val = classNumber;
                    } else if (inputArg === protoBoolean) {
                        val = classBoolean;
                    } else if (inputArg === protoObject) {
                        val = classObject;
                    } else if (inputArg === protoFunction) {
                        val = classFunction;
                    } else if (inputArg === protoString) {
                        val = classString;
                    } else if (inputArg === protoDate) {
                        val = classDate;
                    } else if (inputArg === protoRegExp) {
                        val = classRegexp;
    ...
    */
    ...
    $.Boolean.isTrueAny = function (inputArg) {
        return toClass(inputArg) === classBoolean && base.Boolean.valueOf.call(inputArg);
    };
    ...
    $.Number.outRange = function (value, min, max) {
        if ((toClass(value) === classNumber && !base.isNaN(value)) || isNumeric(value)) {
            value = +value;
        } else {
            return true;
        }

        if ((toClass(min) === classNumber && !base.isNaN(min)) || isNumeric(min)) {
            min = +min;
        } else {
            return true;
        }

        if ((toClass(max) === classNumber && !base.isNaN(min)) || isNumeric(max)) {
            max = +max;
        } else {
            return true;
        }
    ...
    if (!testShims && isNative(mStringify)) {
        ...
        try {
            testTemp.stringifySupport =
                ...
                $.Object.isUndefined(mStringify($.Function.noop)) &&
                ...
                mStringify([Undefined, $.Function.noop, null]) === '[null,null,null]' &&

...
(function (globalThis, window, JSON, module, define) {
    ...
    var nativ = {
        ...
        noNewObject = CtrObject,
        prototypeOfObject = CtrObject.prototype,
        isIsFn = CtrObject.is,
        ...
        fillFN = baseArray.fill,
        copyWithinFN = baseArray.copyWithin,
        findFN = baseArray.find,
        findIndexFN = baseArray.findIndex,
        CtrArray = baseArray.constructor,
        ofFN = CtrArray.of,
        fromFN = CtrArray.from,
        ...
        noNewString = CtrString,
        prototypeOfString = CtrString.prototype,
        ...
        prototypeOfNumber = CtrNumber.prototype,

        baseBoolean = true,
        CtrBoolean = baseBoolean.constructor,
        prototypeOfBoolean = CtrBoolean.prototype,

        baseFunction = function () {
            return;
        },
        tostringFnFN = baseFunction.toString,
        bindFN = baseFunction.bind,
        CtrFunction = baseFunction.constructor,
        noNewFunction = CtrFunction,
        prototypeOfFunction = CtrFunction.prototype,

        baseRegExp = new RegExp(),
        execFN = baseRegExp.exec,
        testFN = baseRegExp.test,
        CtrRegExp = baseRegExp.constructor,
        prototypeOfRegExp = CtrRegExp.prototype,
        correctExecNpcg,

        baseDate = new Date(),
        CtrDate = baseDate.constructor,
        prototypeOfDate = CtrDate.prototype,

        baseError = new Error(),
        CtrError = baseError.constructor,
        prototypeOfError = CtrError.prototype,

        baseTypeError = new TypeError(),
        CtrTypeError = baseTypeError.constructor,
        prototypeOfTypeError = CtrTypeError.prototype,

        baseSyntaxError = new SyntaxError(),
        CtrSyntaxError = baseSyntaxError.constructor,
        prototypeOfSyntaxError = CtrSyntaxError.prototype,

        baseRangeError = new RangeError(),
        CtrRangeError = baseRangeError.constructor,
        prototypeOfRangeError = CtrRangeError.prototype,

        baseEvalError = new EvalError(),
        CtrEvalError = baseEvalError.constructor,
        prototypeOfEvalError = CtrEvalError.prototype,

        baseReferenceError = new ReferenceError(),
        CtrReferenceError = baseReferenceError.constructor,
        prototypeOfReferenceError = CtrReferenceError.prototype,

        baseURIError = new URIError(),
    ...
    $.argToObject = function (inputArg) {
        return noNewObject($.checkObjectCoercible(inputArg));
    };
    ...
    isErrorTypePrototype = function (inputArg) {
        ...
        switch (inputArg) {
        case prototypeOfError:
            /* falls through */
        case prototypeOfTypeError:
            /* falls through */
        case prototypeOfSyntaxError:
            /* falls through */
        case prototypeOfRangeError:
            /* falls through */
        case prototypeOfEvalError:
            /* falls through */
        case prototypeOfReferenceError:
            /* falls through */
        case prototypeOfURIError:
            result = true;
            break;
        default:
            result = false;
    ...
    try {
        if ($.strictEqual(toStringFN.call(), className.Undefined) &&
                ...
                $.strictEqual(toStringFN.call(prototypeOfRegExp), className.regexp) &&
    ...
    try {
        ...
        if ($.isTypeObject(window) && $.isTruthy(window.alert)) {
    ...
    function throwIfNotAFunction(inputArg) {
        if (!$.isFunction(inputArg)) {
            throw new CtrTypeError(inputArg + ' is not a function');
        }
    ...
    function throwIfIsNotTypeObjectOrIsNotFunction(inputArg) {
        if (!isTypeObjectOrIsFunction(inputArg)) {
            throw new CtrTypeError('called on non-object');
        }
     ...
     */
    try {
        if (testShims || !$.isNativeFunction(bindFN)) {
            throw new CtrError();
        }

        testObject1 = [1, 2, 3];
        TestConstructor = bindFN.call(function () {
            return testObject1;
        }, null);

        testObject2 = new TestConstructor();
        if ($.notStrictEqual(testObject1, testObject2)) {
            throw new CtrError();
        }
        ...
        TestConstructor = bindFN.call(testObject1, null, 'B');
        testObject2 = new TestConstructor();
        ...
        $.bind = function (fn, thisArg) {
            return bindFN.apply(fn, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } catch (e) {
        BindCtor = function () {
        ...
        $.bind = function (fn, thisArg) {
            ...
            var args = $.arraySlice(arguments, 2),
                bound,
                binder = function () {
                    if ($.objectInstanceOf(this, bound)) {
                        var result = fn.apply(this, args.concat($.arraySlice(arguments)));

                        if ($.strictEqual(noNewObject(result), result)) {
                            return result;
                        }
                    ...
                    return fn.apply(thisArg, args.concat($.arraySlice(arguments)));
                },
                boundLength = nativ.Math.max($.POSITIVE_ZERO, fn.length - args.length),
                boundArgs = [],
                index;

            for (index = $.POSITIVE_ZERO; $.lt(index, boundLength); index += 1) {
                $.arrayPush(boundArgs, '$' + index);
            }

            bound = noNewFunction('binder', 'return function(' + $.arrayJoin(boundArgs, ',') +
                             '){return binder.apply(this,arguments)}')(binder);

            if ($.isPlainObject(fn.prototype)) {
                BindCtor.prototype = fn.prototype;
                bound.prototype = new BindCtor();
    ...
    if (!testShims && $.isNativeFunction(isArrayFN)) {
    ...
    if (!testShims && $.isNativeFunction(isIsFn)) {
        $.objectIs = isIsFn;
    } else {
    ...
    $.areSameTypeOf = function (a, b) {
        ...
        if ($.lt(length, 2)) {
            throw new CtrSyntaxError('must supply at least 2 arguments');
        }
    ...
    function inRange(value, min, max) {
        if (!$.isNumber(value) && !$.isString(value)) {
            throw new CtrTypeError('arguments must be either numbers or strings');
        }

        if ($.strictEqual(min, max) || $.numberIsNaN(min) || $.numberIsNaN(max)) {
            throw new CtrRangeError('min and max do not define a range');
        }
    ...
    $.inRange = function (value, min, max) {
        if (!$.areSameTypeOf(value, min, max)) {
            throw new CtrTypeError('arguments must be of the same type');
        }
    ...
    if (!testShims && $.isNativeFunction(isNaNFN)) {
        $.numberIsNaN = isNaNFN;
    } else {
    ...
    if (!testShims && $.isNativeFunction(isFiniteFN)) {
    ...
    if (!testShims && $.isNativeFunction(nativ.Math.sign)) {
    ...
    if (!testShims && $.isNativeFunction(toIntegerFN)) {
    ...
    try {
        if (testShims || !$.isNativeFunction(isIntegerFN)) {
            throw new CtrError();
        }
    ...
    function throwIfIsNotRegExp(inputArg) {
        if (!$.isRegExp(inputArg)) {
            throw new CtrTypeError('Type RegExp expected');
        }
        ...
        if ($.isFunction(replacement)) {
            ...
            result = replaceFN.call(str, search, function () {
                ...
                if ($.isTrue(isRegex) && $.isTrue(search.global)) {
                    search.lastIndex = args[args.length - 2] + $.arrayFirst(args).length;
                }

                // Should pass `undefined` as context; see
                // <https://bugs.ecmascript.org/show_bug.cgi?id=154>
                return replacement.apply($.privateUndefined, args);
            });
        } else {
    ...
    if (!testShims && $.isNativeFunction(repeatFN)) {
        /*jshint -W098 */
        $.stringRepeat = function (string, times) {
            return repeatFN.apply(string, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.stringRepeat = function (string, times) {
            var thisString = onlyCoercibleToString(string),
                count = $.numberToInteger(times);

            if (isLtZeroOrPositiveInfinity(count)) {
                throw new CtrRangeError();
            }
    ...
    if (!testShims && $.isNativeFunction(startsWithFN)) {
        /*jshint -W098 */
        $.stringStartsWith = function (string, searchString, position) {
            return startsWithFN.apply(string, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.stringStartsWith = function (string, searchString, position) {
    ...
    if (!testShims && $.isNativeFunction(endsWithFN)) {
        /*jshint -W098 */
        $.stringEndsWith = function (string, searchString, position) {
            return endsWithFN.apply(string, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.stringEndsWith = function (string, searchString, position) {
    ...
    if (!testShims && $.isNativeFunction(containsFN)) {
        /*jshint -W098 */
        $.stringContains = function (string, searchString, position) {
            return containsFN.apply(string, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.stringContains = function (string, searchString, position) {
    ...
    if (!testShims && $.isNativeFunction(getPrototypeOfFN)) {
        ...
        $.objectGetPrototypeOf = function (object) {
            if ($.strictEqual(throwIfIsNotTypeObjectOrIsNotFunction(object), prototypeOfObject)) {
                return null;
            }
    ...
    function throwIfIsNotTypeObjectOrIsFunction(inputArg) {
        if (isNotTypeObjectOrIsFunction(inputArg)) {
            throw new CtrTypeError('called on a invalid object');
        }
    ...
    function throwIfIsNotHasValidLength(inputArg) {
        if (!hasValidLength(inputArg)) {
            throw new CtrTypeError('invalid length property');
        }
    ...
    $.arrayAssign = function (array, index, value) {
        ...
        if ($.gte(arguments.length, 3)) {
            if ($.numberIsFinite($.toNumber(index))) {
    ...
    if (!testShims && $.isNativeFunction(spliceFN) &&
            $.strictEqual(spliceFN.call([1, 2], $.POSITIVE_ZERO).length, 2)) {
    ...
    try {
        ...
        if (!testShims && $.isNativeFunction(forEachFN) && $.isTypeObject(testObject1) &&
    ...
    try {
        ...
        if (!testShims && $.isNativeFunction(someFN) && $.isTypeObject(testObject1) &&
                $.strictEqual($.toObjectString(testObject1), className.string)) {
     ...
     * @return {*}
     */
    if (!testShims && $.isNativeFunction(findFN)) {
        /*jshint -W098 */
        $.arrayFind = function (array, fn, thisArg) {
            return findFN.apply(array, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.arrayFind = function (array, fn, thisArg) {
            ...
            for (index = $.POSITIVE_ZERO; $.lt(index, length); index += 1) {
                ...
                if ($.hasProperty(object, index) && fn.call(thisArg, element, index, object)) {
                    val = element;
                    break;
                }
            }
     ...
     * @return {number}
     */
    if (!testShims && $.isNativeFunction(findIndexFN)) {
        /*jshint -W098 */
        $.arrayFindIndex = function (array, fn, thisArg) {
            return findIndexFN.apply(array, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.arrayFindIndex = function (array, fn, thisArg) {
     ...
     * @return {array}
     */
    if (!testShims && $.isNativeFunction(ofFN)) {
        $.arrayOf = ofFN;
    } else {
        $.arrayOf = function () {
            return $.arraySlice(arguments);
        };
    }
     ...
     * @return {array}
     */
    if (!testShims && $.isNativeFunction(fromFN)) {
        $.arrayFrom = fromFN;
    } else {
        $.arrayFrom = function (arrayLike, mapfn, thisArg) {
    ...
    // named $.arraySome instead of some because of SpiderMonkey and Blackberry bug
    try {
        everyFN.call('foo', function (item, index, list) {
            /*jslint unparam: true */
            /*jshint unused: true */
            testObject1 = list;
        });

        if (!testShims && $.isNativeFunction(everyFN) && $.isTypeObject(testObject1) &&
                $.strictEqual($.toObjectString(testObject1), className.string)) {

            /*jshint -W098 */
            $.arrayEvery = function (array, fn, thisArg) {
                return everyFN.apply(array, $.arraySlice(arguments, 1));
            };
            /*jshint +W098 */
        } else {
            throw new CtrError();
        ...
        $.arrayEvery = function (array, fn, thisArg) {
            ...
            val = true;
            for (index = $.POSITIVE_ZERO; $.lt(index, length); index += 1) {
                if ($.hasProperty(object, index) && $.isFalsy(fn.call(thisArg, object[index], index, object))) {
                    val = false;
                    break;
                }
            }
    ...
    try {
        ...
        if (!testShims && $.isNativeFunction(mapFN) && $.isTypeObject(testObject1) &&
                $.strictEqual($.toObjectString(testObject1), className.string)) {
     ...
     * @return {array}
     */
    function internalSlice(array, start, end) {
        var object = $.toObjectFixIndexedAccess(array),
            length = clampInteger(object.length, $.POSITIVE_ZERO, $.MAX_UINT32),
     ...
     */
    try {
        if (testShims || !$.isNativeFunction(sliceFN) ||
                $.notStrictEqual(sliceFN.call(someArgs()).toString(), ',,1,a,2,b,,') ||
        ...
        $.arraySlice = function (array, start, end) {
            try {
                ...
                return sliceFN.apply(array, args);
            } catch (e) {
                return internalSlice(array, start, end);
            }
        };
    } catch (e) {
        $.arraySlice = internalSlice;
    }
    ...
    try {
        ...
        if (!testShims && $.isNativeFunction(filterFN) && $.isTypeObject(testObject1) &&
                $.strictEqual($.toObjectString(testObject1), className.string)) {
    ...
    try {
        ...
        if (!testShims && $.isNativeFunction(reduceFN) && $.isTypeObject(testObject1) &&
                $.strictEqual($.toObjectString(testObject1), className.string)) {
        ...
        $.arrayReduce = function (array, fn, initialValue) {
            ...
            if ($.isZero(length) && $.strictEqual(arguments.length, 2)) {
                throw new CtrTypeError(reduceTypeErrorMessage);
            }
    ...
    $.arrayStableSort = function (array, comparefn) {
        ...
        if ($.gt(clampInteger(object.length, $.POSITIVE_ZERO, $.MAX_UINT32), 1)) {
            isTargetArrayOrArguments = isArrayOrArguments(object);
            $.arrayForEach(mergeSort(object, comparefn), function (value, index) {
                if (isTargetArrayOrArguments) {
                    $.arrayAssign(array, index, value);
                } else {
                    array[index] = value;
     ...
     * @return {array}
     */
    if (!testShims && $.isNativeFunction(sortFN)) {
    ...
    try {
        if (!testShims && $.isNativeFunction(trimFN) &&
    ...
    if (!testShims && $.isNativeFunction(indexOfFN) &&
            $.strictEqual(indexOfFN.call([$.POSITIVE_ZERO, 1], 1, 2), -1)) {

        /*jshint -W098 */
        $.arrayIndexOf = function (array, searchElement, fromIndex) {
            return indexOfFN.apply(array, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.arrayIndexOf = function (array, searchElement, fromIndex) {
    ...
    if (!testShims && $.isNativeFunction(indexOfFN) &&
            $.strictEqual(indexOfFN.call([$.POSITIVE_ZERO, 1], $.POSITIVE_ZERO, -3), -1)) {

        /*jshint -W098 */
        $.arrayLastIndexOf = function (array, searchElement, fromIndex) {
            return lastIndexOfFN.apply(array, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
     ...
     * @return {array}
     */
    if (!testShims && $.isNativeFunction(fillFN)) {
        /*jshint -W098 */
        $.arrayFill = function (array, value, start, end) {
            return fillFN.apply(array, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
    } else {
        $.arrayFill = function (array, value, start, end) {
            ...
            if ($.lt(start, $.POSITIVE_ZERO)) {
                relativeStart = nativ.Math.max(length + relativeStart, $.POSITIVE_ZERO);
            } else {
                relativeStart = nativ.Math.min(relativeStart, length);
            }
            ...
            if ($.lt(relativeEnd, $.POSITIVE_ZERO)) {
                final = nativ.Math.max(length + relativeEnd, $.POSITIVE_ZERO);
            } else {
                final = nativ.Math.min(relativeEnd, length);
            }
            ...
            isTargetArrayOrArguments = isArrayOrArguments(object);
            for (index = relativeStart; $.lt(index, final); index += 1) {
                if (isTargetArrayOrArguments) {
                    $.arrayAssign(object, index, value);
                } else {
                    object[index] = value;
                }
            }
     ...
     * @return {array}
     */
    if (!testShims && $.isNativeFunction(copyWithinFN)) {
        /*jshint -W098 */
        $.arrayCopyWithin = function (array, target, start, end) {
            return copyWithinFN.apply(array, $.arraySlice(arguments, 1));
        };
        /*jshint +W098 */
        ...
        $.arrayCopyWithin = function (array, target, start, end) {
            var object = $.toObjectFixIndexedAccess(array),
                length = clampInteger(object.length, $.POSITIVE_ZERO, $.MAX_UINT32),
                relativeTarget = $.numberToInteger(target),
                relativeStart = $.numberToInteger(start),
                relativeEnd,
                to,
                from,
            ...
            if ($.lt(relativeTarget, $.POSITIVE_ZERO)) {
                to = nativ.Math.max(length + relativeTarget, $.POSITIVE_ZERO);
            } else {
                to = nativ.Math.min(relativeTarget, length);
            }

            if ($.lt(relativeStart, $.POSITIVE_ZERO)) {
                from = nativ.Math.max(length + relativeStart, $.POSITIVE_ZERO);
            } else {
                from = nativ.Math.min(relativeStart, length);
            }
            ...
            count = nativ.Math.min(final - from, length - to);
            if ($.lt(from, to) && $.lt(to, from + count)) {
                direction = -1;
                from += count - 1;
                to += count - 1;
            } else {
            ...
            isTargetArrayOrArguments = isArrayOrArguments(object);
            for (index = count; $.gt(index, $.POSITIVE_ZERO); index -= 1) {
                if ($.objectHasOwnProperty(object, from)) {
                    if (isTargetArrayOrArguments) {
                        $.arrayAssign(object, to, object[from]);
                    } else {
                        object[to] = object[from];
                    }
                } else {
    ...
    try {
        if (!testShims && $.isNativeFunction(keysFN)) {
            if ($.isTrue(hasErrorProps)) {
        ...
        $.objectKeys = function (object) {
            ...
            for (prop in object) {
                if (!($.isTrue(skipProto) && $.strictEqual(prop, strings.Prototype)) &&
                        !($.isTrue(skipErrorProps) && $.arrayContains(unwantedErrorProps, prop)) &&
    ...
    try {
        ...
        try {
            ...
            if (!$.isNull(testValue[1])) {
                throw new CtrError('Fails integer check');
            }
        } catch (e) {
        ...
        try {
            ...
            if (!$.isNull(testObject1[$.POSITIVE_ZERO])) {
                throw new CtrError('Fails array check');
            }
        } catch (e) {
        ...
        if ($.isTrue(definePropertyPatch1) || $.isTrue(definePropertyPatch2) || $.isTrue(definePropertyPatch3)) {
    ...
    $.objectDefineProperties = function (object, props) {
        ...
        if (!isTypeObjectOrIsFunction(props)) {
            throw new CtrTypeError('Property description must be an object');
        }
    ...
    if (!testShims && $.isNativeFunction(freezeFN)) {
    ...
    if (!testShims && $.isNativeFunction(isFrozenFN)) {
    ...
    if (!testShims && $.isNativeFunction(isPrototypeOfFN)) {
    ...
    $.isPlainObject = function (object) {
        return $.isTypeObject(object) && $.isObject(object) &&
            $.strictEqual($.objectGetPrototypeOf(object), prototypeOfObject);
    };
     ...
     * @return {object}
     */
    if (!testShims && $.isNativeFunction(assignFN)) {
        $.objectAssign = assignFN;
    } else {
        $.objectAssign = function (target) {
            ...
            $.arrayForEach($.arraySlice(arguments, 1), function (source) {
                $.arrayForEach($.objectKeys(throwIfIsNotTypeObjectOrIsNotFunction(source)), function (key) {
                    var value = source[key];

                    if (isTargetArrayOrArguments) {
                        $.arrayAssign(target, key, value);
                    } else {
                        target[key] = value;
                    }
                });
    ...
    try {
        ...
        $.objectCreate = function (prototype, propertiesObject) {
            ...
            $.objectDefineProperty(newObject, strings.proto, $.objectAssign({
                value: prototype
            }, notEnumerableProperties));
    ...
    $.deepEqual = function (a, b, opts) {
        ...
        if ($.isTrue(opts.strict)) {
            if (!$.objectIs($.objectGetPrototypeOf($.argToObject(a)),
                                $.objectGetPrototypeOf($.argToObject(b)))) {
    ...
    $.inherits = function (ctor, superCtor) {
        ...
        $.objectDefineProperty(ctor.prototype, strings.Constructor, $.objectAssign({
            value: ctor
        }, notEnumerableProperties));
    };
    ...
    function makeCustomError(name, ErrorConstructor, maxMessageLength) {
        if (!$.isString(name) || $.isEmptyString(name)) {
            throw new CtrTypeError('"name" was not a valid string');
        }

        if (!$.isErrorTypeConstructor(ErrorConstructor)) {
            throw new CtrTypeError('"ErrorConstructor" was not an Error type');
        }
        ...
        function CustomError(message, stackStartFunction) {
            ...
            $.objectDefineProperty(this, strings.message, $.objectAssign({
                value: message
            }, notEnumerableProperties));
            ...
            if ($.isFunction(ErrorConstructor.captureStackTrace)) {
                ...
                if ($.isString(err[strings.stack])) {
                    $.objectDefineProperty(this, strings.stack, $.objectAssign({
    ...
    // and Blackberry bug
    try {
        if (testShims || !$.isNativeFunction(getOwnPropDescFN)) {
            throw new CtrError();
        }
    ...
    if (!testShims && $.isTypeObject(JSON)) {
        ...
        stringifyToString = function (key, holder) {
            ...
            case strings.object:
                ...
                if ($.arrayIsArray(value)) {
                    /*jshint -W098 */
                    partial = $.arrayMap(value, function (element, index) {
                        return stringifyToString(index, value) || strings.Null;
                    });
                    /*jshint +W098 */
                ...
                if ($.arrayIsArray(stringifyReplacer)) {
                    partial = $.arrayReduce(stringifyReplacer, function (prev, element) {
                        var v;

                        if ($.isString(element)) {
                            v = stringifyToString(element, value);
                            if (!$.isUndefined(v)) {
                                $.arrayPush(prev, stringifyQuote(element) +
                    ...
                    partial = $.arrayReduce($.objectKeys(value), function (prev, k) {
                        var v = stringifyToString(k, value);

                        if (!$.isUndefined(v)) {
                            $.arrayPush(prev, stringifyQuote(k) +
                                            ($.isStringNotEmpty(stringifyGap) ? ': ' : ':') + v);
                        }
    ...
    function powerSet(array) {
        ...
        if ($.isZero(array.length)) {
            ...
            val = $.arrayReduce(powerSet(array), function (previous, element) {
                $.arrayPush(previous, element);
                element = $.arraySlice(element);
                $.arrayPush(element, lastElement);
                $.arrayPush(previous, element);
    ...
    $.powerSet = function (array) {
        if (!$.isArray(array)) {
            throw new CtrTypeError('Argument must be an array');
        }
    ...
    publicUtil = $.objectDefineProperty(factory(), strings.factory, $.objectAssign({
        value: function () {
            return $.objectDefineProperty(factory(), strings.factory, $.objectAssign({
    ...
    if ($.isTypeObject(module) && $.isTypeObject(module.exports)) {
        $.objectDefineProperty(module, 'exports', $.objectAssign({
            value: publicUtil
        }, notEnumerableProperties));
        ...
        $.objectDefineProperty(globalThis, 'utilx', $.objectAssign({
            value: publicUtil
        }, notEnumerableProperties));
    }

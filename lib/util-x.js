...
(function (globalThis, Math, JSON, module, define) {
    ...
    if (splitFN.call('ab', new RegExp('(?:ab)*')).length !== 2 ||
            splitFN.call(dotString, new RegExp('(.?)(.?)')).length !== 4 ||
            splitFN.call('tesst', new RegExp('(s)*'))[1] === 't' ||
            splitFN.call(emptyString, new RegExp('.?')).length ||
            splitFN.call(dotString, new RegExp('()()')).length > 1 ||
            splitFN.call(testString, new RegExp('(?:)'), -1).length !== 4) {
        ...
        $.stringSplit = function (stringArg, separator, limit) {
            ...
            if ($.isUndefined(separator) && $.isZero(limit)) {
                ...
                val = splitFN.apply(stringArg, $.arraySlice(arguments, 1));
            }
    ...
    // elements.
    // "0".split(undefined, 0) -> []
    } else if (!$.isZero(splitFN.call('0', $.privateUndefined, 0).length)) {
        /*global console */
        console.log('CHROME');
        $.stringSplit = function (stringArg, separator, limit) {
            var val;
        ...
        try {
            ...
            $.stringSplit = function (stringArg, separator, limit) {
                /*global console */
                console.log('COERCE');
                return splitFN.apply(onlyCoercibleToString(stringArg), $.arraySlice(arguments, 1));
            };
            /* jshint +W098 */
        } catch (e) {
            /* jshint -W098 */
            $.stringSplit = function (stringArg, separator, limit) {
                /*global console */
                console.log('RAW');
                return splitFN.apply(stringArg, $.arraySlice(arguments, 1));
            };
            /* jshint +W098 */
        }
    }
    ...
    if ($.isFunction(indexOfFN) && $.strictEqual(indexOfFN.call([$.POSITIVE_ZERO, 1], $.POSITIVE_ZERO, -3), -1)) {
    ...
    try {
        ...
        $.objectDefineProperty = function (object, property, descriptor) {
            ...
            if (!$.objectHasOwnProperty(descriptor, getString) && !$.objectHasOwnProperty(descriptor, setString)) {
                ...
                if ($.isFunction(descriptor[getString])) {
                    defineGetterFN.call(object, property, descriptor[getString]);
                }

                if ($.isFunction(descriptor[setString])) {

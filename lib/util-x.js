...
(function (globalThis) {
    ...
    function factory() {
        ...
        utilx.arraySlice = (function () {
            ...
            try {
                utilx.deepEqual(sliceFN.apply(someArgs(), []), arr.slice());
                utilx.deepEqual(sliceFN.apply(someArgs(), [-1]), arr.slice(-1));
                utilx.deepEqual(sliceFN.apply(someArgs(), [0]), arr.slice(0));
                utilx.deepEqual(sliceFN.apply(someArgs(), [3]), arr.slice(3));
                utilx.deepEqual(sliceFN.apply(someArgs(), [-1, 4]), arr.slice(-1, 4));
                utilx.deepEqual(sliceFN.apply(someArgs(), [0, 4]), arr.slice(0, 4));
                utilx.deepEqual(sliceFN.apply(someArgs(), [3, 6]), arr.slice(3, 6));
            } catch (e) {
            ...
            if (utilx.isFunction(sliceFN)) {
                tempSafariNFE = function nfeSlice(array, start, end) {
                    ...
                    if (utilx.lt(length, 2)) {
                        ...
                        val = sliceFN.apply(array, [start, end]);
                    }

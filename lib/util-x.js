...
(function (globalThis) {
    ...
    function factory() {
        ...
        /**
         * Pathces Opera Error.prototype.toString to make it function as expected in all other browsers.
         * This is an obtrusive fix.
         * @memberOf utilx
         * @function
         * @return {undefined}
         */
        utilx.normaliseErrorOperaToString = function () {
            // I am unable to make this fix for IE6&7 unobtrusive
            try {
                throw new Error('Should we patch Opera?');
            } catch (e) {
                if (utilx.strictEqual(e.message, 'Should we patch Opera?') &&
                        utilx.strictEqual(e.toString(), '[object Error]') &&
                        utilx.stringContains(e.toString(), 'opera:config#UserPrefs|Exceptions Have Stacktrace')) {

                    utilx.objectDefineProperties(Error.prototype, {
                        toString: {
                            value: function () {
                                var arr = utilx.stringSplit(e.message, rxSplitNewLine);
                                ...
                                return this.name + ': ' + utilx.arrayJoin(arr, '\n');
                            },
                            enumerable: false,
                            writable: true,
                            configurable: true

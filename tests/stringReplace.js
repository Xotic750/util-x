...
(function () {
    ...
    describe('stringReplace', function () {
        ...
        it('should replace the first match only when given a nonglobal regex', function () {
            expect(utilx.stringReplace('aaa', /a/, 'b')).to.be('baa');
        });

        it('should replace all matches when given a global regex', function () {
            expect(utilx.stringReplace('aaa', /a/g, 'b')).to.be('bbb');
        });
        ...
        it('should not type convert a string search pattern to a regex', function () {
            expect(utilx.stringReplace('aaa', 'a(a)', 'b')).to.be('aaa');
            expect(utilx.stringReplace('a(a)a', 'a(a)', 'b')).to.be('ba');
        });

        it('should handle single-digit backreference $1 in the replacement string', function () {
            expect(utilx.stringReplace('aaa', /a(a)/, '$1b')).to.be('aba');

            // Backreference to a nonparticipating capturing group
            expect(utilx.stringReplace('test', /t|(e)/g, '$1')).to.be('es');
        });

        it('should handle double-digit backreferences $01, $10, and $99 in the replacement string', function () {
            expect(utilx.stringReplace('aaa', /a(a)/, '$01b')).to.be('aba');
            expect(utilx.stringReplace('aaa', new RegExp('a' + utilx.stringRepeat('()', 9) + '(a)'), '$10b'))
                .to.be('aba');
            expect(utilx.stringReplace('aaa', new RegExp('a' + utilx.stringRepeat('()', 98) + '(a)'), '$99b'))
                .to.be('aba');
        });

        it('should end backreferences in the replacement string after two digits', function () {
            expect(utilx.stringReplace('aaa', new RegExp('a' + utilx.stringRepeat('()', 99) + '(a)'), '$100b'))
                .to.be('0ba');
        });
        ...
        it('should handle backreference $& in the replacement string', function () {
            expect(utilx.stringReplace('aaa', /aa/, '$&b')).to.be('aaba');
            expect(utilx.stringReplace('aaa', 'aa', '$&b')).to.be('aaba');
        });

        it('should handle right context token $\' in the replacement string', function () {
            expect(utilx.stringReplace('aaa', /aa/, '$\'b')).to.be('aba');
            expect(utilx.stringReplace('aaa', 'aa', '$\'b')).to.be('aba');
        });

        it('should handle left context token $` in the replacement string', function () {
            expect(utilx.stringReplace('xaaa', /aa/, '$`b')).to.be('xxba');
            expect(utilx.stringReplace('xaaa', 'aa', '$`b')).to.be('xxba');
        });

        it('should handle token $$ in the replacement string', function () {
            expect(utilx.stringReplace('aaa', /aa/, '$$b')).to.be('$ba');
            expect(utilx.stringReplace('aaa', 'aa', '$$b')).to.be('$ba');
        });

        it('should allow a function to generate the replacement', function () {
            expect(utilx.stringReplace('aaa', /a/, function () {
                return 'b';
            })).to.be('baa');
            expect(utilx.stringReplace('aaa', /a/g, function () {
                return 'b';
            })).to.be('bbb');
            expect(utilx.stringReplace('aaa', 'a', function () {
                return 'b';
        ...
        it('should allow using backreferences with replacement functions', function () {
            expect(utilx.stringReplace('aaa', /aa/, function ($0) {
                return $0 + 'b';
            })).to.be('aaba');

            /*jshint -W098 */
            expect(utilx.stringReplace('aaa', /a(a)/, function ($0, $1) {
                return $1 + 'b';
            })).to.be('aba');
            /*jshint +W098 */
        ...
        it('should allow using the match position within replacement functions', function () {
            /*jshint -W098 */
            expect(utilx.stringReplace('xaaa', /a/, function ($0, pos) {
                return utilx.anyToString(pos);
            })).to.be('x1aa');

            expect(utilx.stringReplace('xaaa', /a/g, function ($0, pos) {
                return utilx.anyToString(pos);
            })).to.be('x123');

            expect(utilx.stringReplace('xaaa', /(a)/g, function ($0, $1, pos) {
                return utilx.anyToString(pos);
            })).to.be('x123');
        ...
        it('should allow using the source string within replacement functions', function () {
            /*jshint -W098 */
            expect(utilx.stringReplace('xaaa', /a/, function ($0, pos, str) {
                return str;
            })).to.be('xxaaaaa');

            expect(utilx.stringReplace('xaaa', /(a)/, function ($0, $1, pos, str) {
                return str;
            })).to.be('xxaaaaa');
        ...
        it('should return string as the typeof the last argument in replacement functions', function () {
            ...
            /*jshint -W098 */
            expect(utilx.stringReplace('100', /0/, function ($0, pos, str) {
                return typeof str;
            })).to.be('1string0');

            /*jshint -W053 */
            expect(utilx.stringReplace(new String('100'), /0/, function ($0, pos, str) {
                return typeof str;
            })).to.be('1string0');
            /*jshint +W053 */
        ...
        it(msg, function () {
            expect(utilx.stringReplace(0, /^/, '$`')).to.be('0');
        });

        it('should not modify the lastIndex of a nonglobal regex', function () {
            var regex = /x/;

            utilx.stringReplace('123x567', regex, '_');
            ...
            utilx.stringReplace('nomatch', regex, '_');
            expect(regex.lastIndex).to.be(1);
        });
        ...
        it('should ignore lastIndex when setting the search start position', function () {
            utilx.arrayForEach([/x/, /x/g], function (regex) {
                regex.lastIndex = 5;
                expect(utilx.stringReplace('123x567', regex, '_')).to.be('123_567');
            });
        });
        ...
        it('should update lastIndex during replacement iterations', function () {
            ...
            utilx.stringReplace('1x2', regex, function () {
        ...
        it('should convert any provided nonstring search to a string', function () {
            ...
            expect(utilx.stringReplace('undefined')).to.be('undefined');
        });
        ...
        it('should convert any provided nonstring/nonfunction replacement to a string', function () {
            var values = [{
                ...
                target: 'xaaa',
                search: /a/,
                replacement: utilx.privateUndefined,
            ...
            expect(utilx.stringReplace('xaaa', /a/)).to.be('xundefinedaa');
        });
        ...
        it('should convert any nonstring context to a string (except null and undefined)', function () {
            ...
            utilx.arrayForEach(values, function (value) {
                expect(utilx.stringReplace(value, /^/, 'x')).to.be('x' + value);
            });
        });
        ...
        it(msg, function () {
            utilx.arrayForEach([null, utilx.privateUndefined], function (value) {
